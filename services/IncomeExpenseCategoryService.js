...
export class IncomeExpenseCategoryService {
  async createIncomeCategory(title, amount, user_id) {
    ...
    const amountInKZT = convertCurrency(user.preffered_currency, amount, 'KZT');
  ...
  async getIncomeCategories(user_id) {
    ...
    const totalPlan = categories.reduce((sum, c) => sum + c.receiving_plan, 0);
    ...
    const totalConverted = parseFloat(
      convertCurrency('KZT', totalReceived, user.preffered_currency).toFixed(2)
    );
    ...
    return {
      status: 200,
      data: {
        budget: String(planConverted) + currencyInSign[user.preffered_currency],
        total: String(totalConverted) + currencyInSign[user.preffered_currency],
